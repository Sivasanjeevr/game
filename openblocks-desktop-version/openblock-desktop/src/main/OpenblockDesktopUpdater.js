...
class OpenblockDesktopUpdater {
    constructor (webContents, resourceServer) {
        ...
        if (appPath.search(/main/g) !== -1) {
            ...
            // Fix for development environment - don't set update config path in dev mode
            if (process.env.NODE_ENV === 'production') {
                // Only set update config path for production builds
                if (process.platform === 'win32') {
                    autoUpdater.updateConfigPath = path.join(appPath, '../win-unpacked/resources/app-update.yml');
                } else if (process.platform === 'darwin') {
                    autoUpdater.updateConfigPath = path.join(appPath, '../mac/OpenBlockDesktop.app/Contents/Resources/app-update.yml');
                } else if (process.platform === 'linux') {
                    autoUpdater.updateConfigPath = path.join(appPath, '../linux-unpacked/resources/app-update.yml');
                }
            }
        }
    ...
    checkUpdateAtStartup () {
        ...
        if (process.env.NODE_ENV !== 'production') {
            console.log('Skipping update checks in development mode');
            return;
        }
    ...
    reqeustCheckUpdate (_windows) {
        // Skip update checks in development mode
        if (process.env.NODE_ENV !== 'production') {
            console.log('Skipping manual update checks in development mode');
